...
public final class ZipImporter2 extends Thread
{
	...
	public void run()
	{
		...
		try
		{
			while((row = this.readLine()) != null)
			{
				...
				{
					this.header = data;
					ZipImporter2.LOG.debug("Header Line with " + this.header.length + " Fields.");
				}
				else
				{
					this.rowdata = data;
					...
					{
						ZipImporter2.LOG.warn("Row " + this.icount + "is not valid");
					}
				}
	...
	private void setReader()
	{
		...
		{
			ZipImporter2.LOG.error("File for City Import does not exist or can not be read " + this.inFile.getName());
		}
		
		ZipImporter2.LOG.info("Start processing file " + this.inFile.getName());
	...
	public void readFile(final File fn)
	{
		...
		{
			ZipImporter2.LOG.error("File for City Import does not exist or can not be read " + fn.getName());
			return;
		}
		
		ZipImporter2.LOG.info("Start processing file " + fn.getName());
		BufferedReader csvReader = null;
		try
		{
	...
	private boolean isModelValid(final ZipModel line)
	{
		...
		{
			ZipImporter2.LOG.warn("Could not parse valid date " + line.getGilt_ab_date());
		}
	...
	private void upsertCity(final ZipModel line)
	{
		...
		this.dao.save(bean);
		ZipImporter2.LOG.debug("Record written to Database with id: " + bean.getCtyId());
	...
	private int getIndex(final String key)
	{
		...
		ZipImporter2.LOG.error("Field " + key + " not found in csv File");
		return -1;
	}
...
class WorkThread extends Thread
{
	...
	public void run()
	{
		...
		if(ls == null || ls.isEmpty())
		{
			bean = this.getNewBean(this.line);
		}
		else
		{
			bean = this.updateBean(ls.get(0), this.line); // first bean
		}
		dao.save(bean);
		WorkThread.LOG.debug("Record written to Database with id: " + bean.getCtyId());
	}
