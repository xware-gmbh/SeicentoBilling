...
public class ProjectDAO extends JPADAO<Project, Long> {
	 ...
	 * @queryMethod Do not edit, method is generated by editor!
	 */
	public List<Project> findByCostAccountActive(final CostAccount cst) {
		final EntityManager entityManager = em();

		final CriteriaBuilder criteriaBuilder = entityManager.getCriteriaBuilder();

		final ParameterExpression<CostAccount> cstParameter = criteriaBuilder.parameter(CostAccount.class, "cst");

		final CriteriaQuery<Project> criteriaQuery = criteriaBuilder.createQuery(Project.class);

		final Root<Project> root = criteriaQuery.from(Project.class);

		criteriaQuery.where(criteriaBuilder.and(criteriaBuilder.equal(root.get(Project_.costAccount), cstParameter),
				criteriaBuilder.equal(root.get(Project_.proState), criteriaBuilder.literal(1))));

		final TypedQuery<Project> query = entityManager.createQuery(criteriaQuery);
		query.setParameter(cstParameter, cst);
		return query.getResultList();
	}
