...
//		this.tableActivity.addItems(bean.getActivities());
	...
	private Resource getInputStream() {
		final Customer cus = CustomerTabView.this.fieldGroup.getItemDataSource().getBean();
		CustomerTabView.this.vcard = new VcardHandler(cus);
		...
		final StreamResource.StreamSource source = new StreamResource.StreamSource() {
			...
			public InputStream getStream() {
				...
				CustomerTabView.this.vcard.generateVcard();

				FileInputStream inStream = null;
				try {
					inStream = new FileInputStream(CustomerTabView.this.vcard.getFile());
	 ...
	 * @eventHandlerDelegate Do NOT delete, used by UI designer!
	 */
	private void cmbCity_containerItemSetChange(final Container.ItemSetChangeEvent event) {
	...
	private void initUI() {
		...
		this.cmdVcard.setIcon(FontAwesome.DOWNLOAD);
		...
		this.horizontalLayout.addComponent(this.cmdVcard);
		this.horizontalLayout.setComponentAlignment(this.cmdVcard, Alignment.MIDDLE_CENTER);
		...
		this.cmbCity.addItemSetChangeListener(event -> this.cmbCity_containerItemSetChange(event));
	...
	private XdevHorizontalLayout actionLayout, horizontalLayoutCity, horizontalLayoutAddress, horizontalLayoutLink,
			horizontalLayout3, horizontalLayout2, horizontalLayout;
