...
import ch.xwr.seicentobilling.business.helper.MigrateVatR2120;
...
public class CompanyTabView extends XdevView {
	...
	private void cmdAdmin_buttonClick(final Button.ClickEvent event) {
		...
		try {
			...
			final String ret = objman.getRowParameter("seicento", "startup", "migrateVat");

			if (! ret.isEmpty() && ret.equals("true")) {
				final MigrateVatR2120 mig = new MigrateVatR2120();
				mig.migrateVatTabelR2120();
			}
	...
	private XdevButton cmdNew, cmdDelete, cmdReload, cmdInfo, cmdAdmin, cmdSave, cmdReset;
